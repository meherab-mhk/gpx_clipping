...
<body data-spy="scroll" data-target=".navbar" data-offset="50">
  ...
  <!-------------------------Menu part end----------------------------->
    ...
    <div class="container">
        <h2 class="text-center">Images for Order By: <span class="text-primary">{{$order->username}}</span> at  <span class="text-primary">{{$order->created_at}} </h2>
    ...
    @foreach ($images as $image)
        <div class="col-md-3">
          <div class="card text-white  mb-3" style="max-width: 20rem;">
            <div class="card-body">
              <img src="/order_images/{{$image->image}}" class="card-img-top">
              <a href="/order_images/{{$image->image}}" download class="btn btn-primary mt-3">Download Image</a>
            </div>
          </div>
        </div>
    @endforeach
